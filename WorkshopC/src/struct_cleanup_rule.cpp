...
bool StructCleanupRule::ReportKey::operator==(const ReportKey &other) const
{
    return decl == other.decl &&
           location == other.location;
}

std::size_t StructCleanupRule::ReportKeyHash::operator()(const ReportKey &key) const
{
...
StructCleanupRule::CleanupAnalyzer::CleanupAnalyzer(
...
StructCleanupRule::CleanupAnalyzer::Flow StructCleanupRule::CleanupAnalyzer::scanStmt(const Stmt *stmt)
{
    if (!stmt)
        return Flow::Normal;
...
void StructCleanupRule::CleanupAnalyzer::trackVar(
    const VarDecl *var)
{
    if (!var)
        return;
...
void StructCleanupRule::CleanupAnalyzer::finalize(
    Flow functionFlow)
{
...
void StructCleanupRule::CleanupAnalyzer::reportContinueCleanup(
    SourceLocation loc)
{
...
void StructCleanupRule::CleanupAnalyzer::reportBreakCleanup(
    SourceLocation loc)
{
    if (scopes.empty())
        return;
...
StructCleanupRule::CleanupAnalyzer::StateSnapshot StructCleanupRule::CleanupAnalyzer::captureState() const
{
...
void StructCleanupRule::CleanupAnalyzer::restoreState(
...
StructCleanupRule::CleanupAnalyzer::StateSnapshot StructCleanupRule::CleanupAnalyzer::mergeStates(
...
bool StructCleanupRule::isThirdParty(
...
bool StructCleanupRule::shouldIgnore(
...
const RecordDecl *StructCleanupRule::getStructDecl(
    QualType type) const
{
    QualType current =
...
bool StructCleanupRule::isStructType(
    QualType type,
...
bool StructCleanupRule::isInsideHelperFunction(
    const FunctionDecl *function,
...
bool StructCleanupRule::shouldTrackVar(
    const VarDecl *var,
...
std::string StructCleanupRule::getStructName(
    QualType type) const
{
    std::string name;
...
bool StructCleanupRule::isDestroyCall(
    const std::string &name,
    const std::string &structName) const
{
    if (name.empty() ||
...
void StructCleanupRule::reportPendingVarsForScopeExit(
    SourceLocation loc,
...
void StructCleanupRule::reportPendingVarsForExit(
    SourceLocation loc,
...
void StructCleanupRule::reportPendingParams(
    SourceLocation loc,
    const std::vector<TrackedVar> &params,
    std::unordered_set<
        ReportKey,
...
void StructCleanupRule::bindFinder(MatchFinder &finder) {
    finder.addMatcher(
        functionDecl(
            isDefinition(),
...
void StructCleanupRule::finalize()
{
