...
bool StructInitRule::isThirdParty(const std::string &file) const
{
...
bool StructInitRule::shouldIgnore(
...
const RecordDecl *StructInitRule::getStructDecl(
    QualType type,
    bool *isArray) const
{
    if (isArray)
...
bool StructInitRule::isStructType(
    QualType type,
    std::string *name,
    bool *isArray) const
{
...
bool StructInitRule::isInsideHelperFunction(
...
const Expr *StructInitRule::unwrapExpr(const Expr *expr) const
{
    while (expr) {
...
bool StructInitRule::isStructReturnFunction(
    const CallExpr *call,
...
bool StructInitRule::isDirectReturnExpression(
    const CallExpr *call,
...
void StructInitRule::checkReturnFunctionUsage(
    const CallExpr *call,
...
bool StructInitRule::exprIsStructReturnValue(const Expr *expr, std::string *structName) const
{
...
bool StructInitRule::exprIsStructVariableReference(
    const Expr *expr,
    std::string *structName) const
{
    expr = unwrapExpr(expr);
...
bool StructInitRule::exprIsStructValue(const Expr *expr, std::string *structName) const
{
...
const VarDecl *StructInitRule::getReferencedVarDecl(const Expr *expr) const
{
    expr = unwrapExpr(expr);
...
std::string StructInitRule::getAssignmentTargetName(const Expr *expr) const
{
    expr = unwrapExpr(expr);
...
bool StructInitRule::isPointerDereference(const Expr *expr) const
{
    expr = unwrapExpr(expr);
...
void StructInitRule::checkAssignment(
...
void StructInitRule::checkReturnStmt(
...
void StructInitRule::checkRecordDecl(const RecordDecl *recordDecl) const
{
    if (!recordDecl || !recordDecl->isStruct())
        return;
...
void StructInitRule::bindFinder(MatchFinder &finder) {
    finder.addMatcher(
        varDecl(
...
void StructInitRule::run(const MatchFinder::MatchResult &result)
{
